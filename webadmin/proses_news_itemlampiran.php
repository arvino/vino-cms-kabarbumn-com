...
if( $action=='itemlampiran_tambahdata' ){ /* KANAL news TAMBAH DATA */



if( $judul =='' ){

	header("location:news_item_lampiran.php?iditem=$idkonten&pesan_error=PLEASE FILL THE TITLE"); /* Kembali ke halaman  sebelumnya */

}else{

$id = newsItemFile_CreateID( $tbl_newsfile ); 
...
$keyword = $judul;
	...
	/* STATUS TAMPIL ITEM LAMPIRAN news */
...
if( $fileupload == "" ){
		...
		header("location:news_item_lampiran.php?iditem=$idkonten&pesan_error=THERE IS NO FILE TO UPLOAD"); /* Kembali ke halaman  sebelumnya */
...
if (!in_array(end(explode(".", strtolower($nama_file))), $allowedExtensions)) {	
	...
	header("location:news_item_lampiran.php?iditem=$idkonten&pesan_error=FILE TYPE IS NOT PERMIT"); /* Kembali ke halaman  sebelumnya */
	...
	$loc_file = "filemodul/news/file_itemlampiran/$idkonten/";
...
$statuslampiran = "1";
newsItem_UpdateFileLampiran( $tbl_newsfile, $idkonten , $statuslampiran );

newsItemFile_TambahData(
		$tbl_newsfile,
		$id, $idkonten, $urutan,
		$judul, $preview, $isi,
	...
	header("location:news_item_lampiran.php?iditem=$idkonten&pesan_error=SUCCESS ADD DATA !"); /* Kembali ke halaman  sebelumnya */ 
...
newsItemFile_UpdateData(
		...
		$keyword
	 	...
	 	header("location:news_item_lampiran.php?iditem=$idkonten&id=$id&action=EditData&pesan_error=UPDATE SUCCESSFULLY... !."); /* Kembali ke halaman  sebelumnya */
...
if( $action=="itemlampiran_statustampil" ){  
	...
	$detail_itemlampiran = ViewDetail_newsFileLampiran( $tbl_newsfile, $id );
	$idkategori = $row_kanal->id;
	SetStatusTampil_newsItemLampiran( $tbl_newsfile, $id, $statustampil );
	header("location:news_item_lampiran.php?iditem=$iditem&pesan_error=SUCCESS PUBLISH DATA.");  
}
...
if( $action=="itemlampiran_hapusdata" ){  
	...
	Hapus_newsFileLampiran( $tbl_newsfile, $id);
	...
	header("location:news_item_lampiran.php?iditem=$iditem&pesan_error=SUCCESS DELETE DATA." );  
} 

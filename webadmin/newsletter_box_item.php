...
$offset = ($nohalaman - 1) * $dataPerhalaman;
		...
		$no = $offset + 1;
		  ...
		  $kategori_halaman  = "news_subkanal/";
		  ...
		  if($HitungSubKategori==0){/* Jika tida ada sub kanal berita arahkan ke subkanal langsung */
			...
			$detail_kanal_news = Detail_KanalBerita_Publish( $tbl_beritakategori , $idkategori );
			$posisihalaman  = "$dataperhalaman_x/$idkategori/$idkategorisub/" . strtolower($detail_kanal_news->keterangan) . "$extention";

		  }else{
		  	 
			$detail_subkanal_news = Detail_SubKanalBerita_Publish( $tbl_beritakategorisub , $idkategorisub );
			$posisihalaman  = "$dataperhalaman_x/$idkategori/$idkategorisub/" . strtolower($detail_subkanal_news->keterangan) . "$extention";
...
$result_news_item = List_Item_Berita_Publish_view_halaman ( $tbl_berita, $idkategori, $idsubkategori,$offset, $dataPerhalaman );
...
<?php 
$hari_berita = harienglish($row_news_item->tgltampil);
$tanggal_berita = bulanenglish($row_news_item->tgltampil);
$tanggal_itemberita = $hari_berita . ", " . $tanggal_berita;
?>
							
<?php 
echo $tanggal_itemberita  ?>  |  <?php 
echo $row_news_item->jamtampil ?> WIB 
...
echo $row_news_item->gambarkecil ?>" width="150" height="100" hspace="5" vspace="5" border="0" align="left"> 
</a>
							<?php 
}else{
							...
							<?php 
echo substr($row_news_item->preview,0,200); ?>...
							</div>

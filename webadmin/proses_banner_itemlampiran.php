...
if( $action=='itemlampiran_tambahdata' ){ /* KANAL banner TAMBAH DATA */



if( $judul =='' ){

	header("location:banner_item_lampiran.php?iditem=$idkonten&pesan_error=PLEASE FILL THE TITLE"); /* Kembali ke halaman  sebelumnya */

}else{

$id = bannerItemFile_CreateID( $tbl_bannerfile ); 
...
$keyword = $judul;
	...
	/* STATUS TAMPIL ITEM LAMPIRAN banner */
...
if( $fileupload == "" ){
		...
		header("location:banner_item_lampiran.php?iditem=$idkonten&pesan_error=THERE IS NO FILE TO UPLOAD"); /* Kembali ke halaman  sebelumnya */
...
if (!in_array(end(explode(".", strtolower($nama_file))), $allowedExtensions)) {	
	...
	header("location:banner_item_lampiran.php?iditem=$idkonten&pesan_error=FILE TYPE IS NOT PERMIT"); /* Kembali ke halaman  sebelumnya */
	...
	$loc_file = "filemodul/banner/file_itemlampiran/$idkonten/";
...
$statuslampiran = "1";
bannerItem_UpdateFileLampiran( $tbl_bannerfile, $idkonten , $statuslampiran );

bannerItemFile_TambahData(
		$tbl_bannerfile,
		$id, $idkonten, $urutan,
		$judul, $preview, $isi,
	...
	header("location:banner_item_lampiran.php?iditem=$idkonten&pesan_error=SUCCESS ADD DATA !"); /* Kembali ke halaman  sebelumnya */ 
...
bannerItemFile_UpdateData(
		...
		$keyword
	 	...
	 	header("location:banner_item_lampiran.php?iditem=$idkonten&id=$id&action=EditData&pesan_error=UPDATE SUCCESSFULLY... !."); /* Kembali ke halaman  sebelumnya */
...
if( $action=="itemlampiran_statustampil" ){  
	...
	SetStatusTampil_bannerItemLampiran( $tbl_bannerfile, $id, $statustampil );
	header("location:banner_item_lampiran.php?iditem=$iditem&pesan_error=SUCCESS PUBLISH DATA.");  
}
...
if( $action=="itemlampiran_hapusdata" ){  
	...
	Hapus_bannerFileLampiran( $tbl_bannerfile, $id);
	...
	header("location:banner_item_lampiran.php?iditem=$iditem&pesan_error=SUCCESS DELETE DATA." );  
} 

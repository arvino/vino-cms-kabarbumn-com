...
if( $keterangan=='' ){
	...
	header("location:news_kanal.php?idkategori=$idkategori&mode=SubSection&pesan_error=PLEASE FILL TITLE"); /* Kembali ke halaman  sebelumnya */
	
}else{

$id = newsKategoriSub_CreateID( $tbl_newskategorisub ); 
$idupline = $idkategori;
...
$sql_hitungdata = newsKategoriSub_PeriksaJudul( $tbl_newskategorisub, $idkategori, $keterangan, $keteranganinggris ); /* Periksa Judul Sub Kategori news, Apakah masih tersedia. */
$hitung_data = mysql_num_rows($sql_hitungdata);
if( $hitung_data > 0 ){ /* Jika judul sudah pernah there is CANCELkan proses tambah data. */

	header("location:news_kanal.php?idkategori=$idkategori&mode=SubSection&pesan_error=FAILED ADD SUB SECTION  !,THE TITLE IS ALREADY EXIST !"); /* Kembali ke halaman  sebelumnya */
...
if( $image_file_header == "" ){ /* JIka tida ada upload file header */
	...
	$location_dir =  $loc_root . $loc_file;
		...
		$uploaddir = $location_dir;
		$new_name = "news_img_header_" . $keterangan;
		$imagefile =  $loc_file;
...
if( $image_file_background == "" ){ /* JIka tida ada upload file background */
	...
	$location_dir =  $loc_root . $loc_file;
		...
		$uploaddir = $location_dir;
		$new_name = "news_img_background_" . $keterangan;
		$imagefile =  $loc_file;
...
/* Tambah data ke Database */
	...
	header("location:news_kanal.php?idkategori=$idkategori&mode=SubSection&pesan_error=SUCCESS ADD DATA !"); /* Kembali ke halaman  sebelumnya */
...
$idkategori = $_POST['idkategori'];
	if( $keterangan=='' ){
		header("location:news_kanal.php?idkategori=$idkategori&mode=SubSection&id=$id&action=EditData&pesan_error=PLEASE FILL THE TITLE"); /* Kembali ke halaman  sebelumnya */
	}else{ 
...
newsKategoriSub_UpdateData(
		...
		$hit, $linkjudul, $keyword 
	 	...
	 	header("location:news_kanal.php?idkategori=" . $idkategori . "&mode=SubSection&id=" . $id . "&action=EditData&pesan_error=UPDATE SUCCESSFULLY... !."); /* Kembali ke halaman  sebelumnya */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	header("location:news_kanal.php?pesan_error=FAILED ! ACCESS DENIED.");  
}else{
	$id = $_GET['id'];
	...
	newsKategoriSub_DeleteImageLogo( $tbl_newskategorisub, $id );
	header("location:news_kanal.php?action=EditData&idkategori=$idkategori&mode=SubSection&id=$id&pesan_error=SUCCESS DELETE IMAGE DATA"); 
}
}
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	//@//unlink($image_subkanal);
	newsKategoriSub_DeleteImageHeader( $tbl_newskategorisub, $id );
	header("location:news_kanal.php?action=EditData&idkategori=$idkategori&mode=SubSection&id=$id&pesan_error=SUCCESS DELETE DATA."); 
}
}
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	//@//unlink($image_subkanal);
	newsKategoriSub_DeleteImageBackground( $tbl_newskategorisub, $id );
	header("location:news_kanal.php?action=EditData&idkategori=$idkategori&mode=SubSection&id=$id&pesan_error=SUCCESS DELETE IMAGE DATA"); 
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	$row_kanal = Select_Detail_Kategori_news( $tbl_newskategori, $id );
	$idkategori = $row_kanal->id;
	newsKategori_StatusTampil( $tbl_newskategori, $statustampil, $id );
	header("location:news_kanal.php?idkategori=" . $idkategori . "&mode=SubSection&pesan_error=SUCCESS PUBLISH DATA."); 
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	$row_kanal = Select_Detail_Kategori_news( $tbl_newskategori, $id );
	$idkategori = $row_kanal->id;
	newsKategori_HomehalamanTampil( $tbl_newskategori, $statustampil, $id );
	header("location:news_kanal.php?idkategori=" . $idkategori . "&mode=SubSection&pesan_error=SUCCESS PUBLISH DATA"); 
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	$row_kanal = Select_Detail_Kategori_news( $tbl_newskategori, $id );
	$idkategori = $row_kanal->id;
	newsKategori_menuatas1_tampil( $tbl_newskategori, $statustampil, $id );
header("location:news_kanal.php?idkategori=" . $idkategori . "&mode=SubSection&pesan_error=SUCCESS PUBLISH DATA");  
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	$row_kanal = Select_Detail_Kategori_news( $tbl_newskategori, $id );
	$idkategori = $row_kanal->id;
	newsKategori_MenuTampil2( $tbl_newskategori, $statustampil2, $id );
	header("location:news_kanal.php?idkategori=" . $idkategori . "&mode=SubSection&pesan_error=SUCCESS PUBLISH DATA");  
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if( $action=="subkanal_hapusdata" ){ /* Proses Update Kanal*/
	...
	$image_subkanal = $root_image . $imagefile . $imagelogo;
	//@//unlink($image_subkanal);
	newsKategoriSub_hapusdata( $tbl_newskategorisub, $id );
	header("location:news_kanal.php?idkategori=" . $idkategori . "&mode=SubSection&pesan_error=SUCCESS DELETE DATA."); /* Kembali ke halaman  sebelumnya */

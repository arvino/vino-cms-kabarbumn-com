...
function Ambil_SubKategori(str1,str2){
	...
	xmlhttp.open("GET","otherhalaman_tampilsubkategori.php?idkategori="+str1+"&idkategorisub="+str2,true);
	xmlhttp.send();
}
...
function JavaScript_Konfirmasi_Item( idkategori, idkategorisub, id ){
	...
	if (answer)
	window.location="proses_otherhalaman_item.php?action=item_hapusdata&idkategori=" + idkategori + "&idkategorisub=" + idkategorisub + "&id=" + id
	else
	window.location="otherhalaman_item.php?idkategori=" + idkategori + "&idsubkategori=" + idkategorisub + "&id=" + id + "&action=ViewList"
}
...
function JavaScript_Konfirmasi_ItemLampiran( iditem , id ){
	var answer = confirm ("Are you sure want to delete this file.")
	if (answer)
	window.location="proses_otherhalaman_itemlampiran.php?action=itemlampiran_hapusdata&iditem="+  iditem  +"&id=" + id 
	else
	window.location="otherhalaman_item_lampiran.php?iditem=" + iditem
}
...
function JavaScript_Konfirmasi_ItemKomentar( id, iditem ){
	var answer = confirm ("Are you sure want to delete this file.")
	if (answer)
	window.location="proses_otherhalaman_itemkomentar.php?action=itemkomentar_hapusdata&iditem="+  iditem  +"&id=" + id 
	else
	window.location="otherhalaman_item_komentar.php?iditem=" + iditem
}
...
<span class="JudulKanal_box1"> halaman  PENGELOLAAN ORDER PRODUK </span>
<hr class='line_box'>
...
<?php 
$KodeKeamananhalaman  = "token_booking";
...
/* Kata Pengantar eotherhalaman */
if(!isset($action)){
include"otherhalaman_pengantar.php";
}
//include"otherhalaman_list_item.php";

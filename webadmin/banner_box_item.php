...
$offset = ($nohalaman - 1) * $dataPerhalaman;
		...
		$no = $offset + 1;
		  ...
		  $kategori_halaman  = "banner_subkanal/";
		  ...
		  if($HitungSubKategori==0){/* Jika tida ada sub kanal berita arahkan ke subkanal langsung */
			...
			$detail_kanal_banner = Detail_KanalBerita_Publish( $tbl_beritakategori , $idkategori );
			$posisihalaman  = "$dataperhalaman_x/$idkategori/$idkategorisub/" . strtolower($detail_kanal_banner->keterangan) . "$extention";

		  }else{
		  	 
			$detail_subkanal_banner = Detail_SubKanalBerita_Publish( $tbl_beritakategorisub , $idkategorisub );
			$posisihalaman  = "$dataperhalaman_x/$idkategori/$idkategorisub/" . strtolower($detail_subkanal_banner->keterangan) . "$extention";
...
$result_banner_item = List_Item_Berita_Publish_view_halaman ( $tbl_berita, $idkategori, $idsubkategori,$offset, $dataPerhalaman );
...
$Link_Judul = potong_judul($row_banner_item->judul);
$id = $row_banner_item->id;
$idkategori = $row_banner_item->idkategori;
$idkategorisub = $row_banner_item->idkategorisub;

$HitungSubKategori = Count_KategoriSub_ByKategori( $tbl_beritakategorisub, $idkategori);
if($HitungSubKategori==0){/* Jika tida ada sub kanal berita arahkan ke subkanal langsung */
		  	
$detail_kanal_banner = Detail_KanalBerita_Publish( $tbl_beritakategori , $idkategori );
$SubKanal_Keterangan = strtolower($detail_kanal_banner->keterangan);

}else{
		  	 
$detail_subkanal_banner = Detail_SubKanalBerita_Publish( $tbl_beritakategorisub , $idkategorisub );
$SubKanal_Keterangan = strtolower($detail_subkanal_banner->keterangan);
...
<a href='<?php 
echo $link_host ?>baca/banner/<?php 
echo $idkategori ?>/<?php 
...
<?php 
$hari_berita = harienglish($row_banner_item->tgltampil);
$tanggal_berita = bulanenglish($row_banner_item->tgltampil);
$tanggal_itemberita = $hari_berita . ", " . $tanggal_berita;
?>
							
<?php 
echo $tanggal_itemberita  ?>  |  <?php 
echo $row_banner_item->jamtampil ?> WIB 
							
</div>
<?php 
if($row_banner_item->gambarbesar !=''){
?>
...
echo $row_banner_item->gambarkecil ?>" width="150" height="100" hspace="5" vspace="5" border="0" align="left"> 
</a>
							<?php 
}else{
							...
							<?php 
echo substr($row_banner_item->preview,0,200); ?>...
							</div>

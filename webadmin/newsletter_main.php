...
function JavaScript_Konfirmasi_Item( id ){
	...
	if (answer)
	window.location="newsletter_proses_item.php?action=item_hapusdata&id=" + id
	else
	window.location="newsletter_item.php?id=" + id + "&action=ViewList"
}
...
<span class="JudulKanal_box1"> NEWSLETTER halaman <?php 
echo strtoupper($detail_kanal->keterangan) ?> </span>
<hr class='line_box'>

<?php 
if(!isset($action)){
	include"newsletter_form_item.php";
	include"newsletter_list_item.php";

...
function Ambil_SubKategori(str1,str2){
	...
	xmlhttp.open("GET","guestbook_tampilsubkategori.php?idkategori="+str1+"&idkategorisub="+str2,true);
	xmlhttp.send();
}
...
function JavaScript_Konfirmasi_Item( idkategori, idkategorisub, id ){
	...
	if (answer)
	window.location="guestbook_proses_item.php?action=item_hapusdata&idkategori=" + idkategori + "&idkategorisub=" + idkategorisub + "&id=" + id
	else
	window.location="guestbook_item.php?idkategori=" + idkategori + "&idsubkategori=" + idkategorisub + "&id=" + id + "&action=ViewList"
}
...
function JavaScript_Konfirmasi_ItemLampiran( iditem , id ){
	var answer = confirm ("Are you sure want to delete this file.")
	if (answer)
	window.location="guestbook_proses_itemlampiran.php?action=itemlampiran_hapusdata&iditem="+  iditem  +"&id=" + id 
	else
	window.location="guestbook_item_lampiran.php?iditem=" + iditem
}
...
function JavaScript_Konfirmasi_ItemKomentar( id, iditem ){
	var answer = confirm ("Are you sure want to delete this file.")
	if (answer)
	window.location="guestbook_proses_itemkomentar.php?action=itemkomentar_hapusdata&iditem="+  iditem  +"&id=" + id 
	else
	window.location="guestbook_item_komentar.php?iditem=" + iditem
}
...
<span class="JudulKanal_box1"> PAGE GUESTBOOK / TESTIMONIAL MANAGER </span>
<hr class='line_box'>
...
/* Kata Pengantar eguestbook */
if(!isset($action)){
include"guestbook_pengantar.php";
}
